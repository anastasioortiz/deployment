...
export default function PokemonFilter(props){
  ...
  useEffect(()=> {
    console.log("update pokemons on filter");
    updatePokemons();
    
  }, [props.typePressed, props.setTypePressed])
...
function updatePokemons(){
    ...
    pokemonData.forEach((item, index) => {
      updatedShowPokemon[index + 1] = checkFilterType(
        item.type,
        props.typePressed
      );
    });
    props.setShowPokemon(updatedShowPokemon);
}
